...
public class MessageRepository(DataContext context, IMapper mapper) : IMessageRepository
{
    ...
    public Task<PagedList<MessageDto>> GetMessagesForUser(MessageParams messageParams)
    {
        ...
        {
            "Inbox" => query.Where(x => x.Reciptent.UserName == messageParams.Username && x.ReciptentDeleted == false),
            "Outbox" => query.Where(x => x.Sender.UserName == messageParams.Username && x.SenderDeleted == false),
            _ => query.Where(x => x.Reciptent.UserName == messageParams.Username && x.DateRead == null && x.ReciptentDeleted == false)
        };
